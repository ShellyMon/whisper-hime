...
namespace SoraBot.Commands
{
    ...
    public partial class Pixiv
    {
        ...
        [SoraCommand(CommandExpressions = new[] { "^\\.pid (\\d+?)$" }, MatchType = Sora.Enumeration.MatchType.Regex, SourceType = SourceFlag.Group, SuperUserCommand = false)]
        public static async ValueTask GetImageByPid(GroupMessageEventArgs ev)
        {
            // 解析参数
            var match = ev.CommandRegex[0].MatchResult(ev.Message.GetText());
        ...
        [SoraCommand(CommandExpressions = new[] { "^\\.add (\\d+?)$" }, MatchType = Sora.Enumeration.MatchType.Regex, SourceType = SourceFlag.Group, SuperUserCommand = false)]
        public static async ValueTask AddImageToDatabaseByPidG(GroupMessageEventArgs ev)
        {
            // 解析参数
            var match = ev.CommandRegex[0].MatchResult(ev.Message.GetText());
